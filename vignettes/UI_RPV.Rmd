---
title:
  formatted: 'Application of the RPV method and other functions of the \pkg{UncertainInterval} package'
...
preamble: |
  \usepackage{amsmath}
abstract: This paper demonstrates the use of the \pkg{UncertainInterval} package for
...
As the Youden threshold maximizes the sum of $Se$ + $Sp$, the results are slightly better:
...
The results for $Sp$ and $Se$ are lower compared to the table based on the optimal dichotomization. This is because for the calculation of $Sp$ and $Se$, the uncertain test scores are all incorrectly considered as unambiguous errors. Especially when using these uncertain scores for a positive or negative classification, many classification errors are made. When these test scores are considered as uncertain, another possible line of action can be chosen. In these cases, choosing a more cautious line of action can reduce over-treatment and treatment errors.
